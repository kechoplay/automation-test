...
export default function Sidebar({ refreshKey, currentTestCaseId, onSelectTestCase }) {
  ...
  const TCItem = ({ tc }) => (
    <div
      ...
      className={`flex items-center gap-1.5 px-2 py-1 rounded cursor-pointer hover:bg-slate-800 ${currentTestCaseId === tc.id ? 'bg-blue-950' : ''}`}
    >
      <span className={`${METHOD_COLOR[tc.method] || 'text-slate-400'} text-xs font-bold w-10 shrink-0`}>{tc.method}</span>
      <span className="text-xs text-slate-300 truncate">{tc.name}</span>
    </div>
  )
  ...
  return (
    <aside className="w-60 shrink-0 flex flex-col border-r border-slate-700/60 bg-slate-900 overflow-hidden">
      <div className="flex items-center justify-between px-3 py-2.5 border-b border-slate-700/60">
        ...
        <button onClick={handleNewFolder} className="w-6 h-6 rounded flex items-center justify-center text-slate-400 hover:text-white hover:bg-slate-700 text-lg leading-none">+</button>
      </div>
